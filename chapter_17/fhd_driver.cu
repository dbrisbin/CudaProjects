                ...
                const int section_size)
{
    ...
    cudaMalloc((void**)&d_k_struct, M * sizeof(KData));
    ...
    cudaMemcpy(d_k_struct, k_struct, M * sizeof(KData), cudaMemcpyHostToDevice);
    ...
    for (int i = 0; i < iters; i++)
    {
        ...
        switch (kernel_to_use)
        {
            ...
            {
#ifdef USE_STRUCT
                std::cout << "Do not define USE_STRUCT to use selected kernel" << std::endl;
                return -1.F;
#endif
                dim3 block_dim(section_size, 1, 1);
                dim3 grid_dim(std::ceil(static_cast<float>(M) / section_size), 1, 1);
            ...
            case FhdKernels::kLoopInterchangeWithRegistersAndConstantMemStruct:
            {
#ifndef USE_STRUCT
                std::cout << "Define USE_STRUCT to use selected kernel" << std::endl;
                return -1.F;
#endif
                dim3 block_dim(section_size, 1, 1);
                dim3 grid_dim(std::ceil(static_cast<float>(M) / section_size), 1, 1);
                ...
                {
                    const auto M_offset = i * kChunkSize;
                    if (cudaMemcpyToSymbol(k_c, &k_struct[M_offset], kChunkSize * sizeof(KData)))
                    {
                        std::cerr << "Error copying to constant memory" << std::endl;
                        return -1.F;
                    }

                    ComputeFHDWithNThreadsRegistersAndConstantMemStruct<<<grid_dim, block_dim>>>(
                        d_x, d_y, d_z, d_r_mu, d_i_mu, static_cast<int>(kChunkSize), N,
                        static_cast<int>(M_offset), d_r_fhd, d_i_fhd);
                }
                if (M % kChunkSize != 0)
                {
                    const auto M_offset = kChunkSize * (M / kChunkSize);
                    const int remaining = M % kChunkSize;
                    if (cudaMemcpyToSymbol(k_c, &k_struct[M_offset], remaining * sizeof(KData)))
                    {
                        std::cerr << "Error copying to constant memory" << std::endl;
                        return -1.F;
                    }
            ...
            case FhdKernels::kLoopInterchangeWithRegistersAndConstantMemStructAndDeviceTrig:
            {
#ifndef USE_STRUCT
                std::cout << "Define USE_STRUCT to use selected kernel" << std::endl;
                return -1.F;
                ...
                for (size_t i{0}; i < M / kChunkSize; i++)
                {
                    ...
                    ComputeFHDWithNThreadsRegistersAndConstantMemStructDeviceTrig<<<grid_dim,
                                                                                    block_dim>>>(
                        d_x, d_y, d_z, d_r_mu, d_i_mu, static_cast<int>(kChunkSize), N,
                        static_cast<int>(M_offset), d_r_fhd, d_i_fhd);
                }
