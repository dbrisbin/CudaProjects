...
{
    float *values_d{}, *vec_d{}, *result_d{};
    int *col_indices_d{}, *row_indices_d{}, *row_pointers_d{};
    ...
    switch (kernel_to_use)
    {
        case SpmvKernel::kCooSpmv:
        {
            ...
            float* values_h = new float[number_of_nnz_elts];
            int* col_indices_h = new int[number_of_nnz_elts];
            int* row_indices_h = new int[number_of_nnz_elts];
        ...
        case SpmvKernel::kCsrSpmv:
        {
            ...
            float* values_h = new float[number_of_nnz_elts];
            int* col_indices_h = new int[number_of_nnz_elts];
            int* row_pointers_h = new int[m + 1];
    ...
    cudaError_t err = cudaMemcpy(result_h, result_d, m * sizeof(float), cudaMemcpyDeviceToHost);
    if (err != cudaSuccess)
    {
